...
static INPUT_FILE: &str = "data/day10/input.txt";
...
impl Day {
    ...
    fn execute(&mut self) {
        ...
        if self.part1 {
            ...
            for l in 0..6 {
                if result2[l] != expected[l] {
                    panic!("Part2 is broken {} != {}", result2[l], expected[l]);
                }
            }
        }
    }
    ...
    fn part1(&self) -> i64 {
        ...
        for i in 0..count {
            ...
            if line[0] == 'n' as u8 {
                ...
                cycle += 1;
                let value: i64 = std::str::from_utf8(&line[5..])
                    .unwrap()
                    .parse()
                    .expect("Not a number");
                reg_x += value;
    ...
    fn part2(&self) -> Vec<String> {
        let count = self.strings.len();
        let mut pixel_pos = 0;
        let mut spirte_left = 0;
        let mut spirte_right = 2;
        let mut screen: Vec<String> = Vec::new();
        screen.resize(6, String::new());
        let mut y = 0;
        for i in 0..count {
            let line = &self.strings[i];
            if spirte_left <= pixel_pos && pixel_pos <= spirte_right {
                screen[y].push('#');
            } else {
                screen[y].push('.');
            }
    ...
    fn part1() {
        let input: (Vec<&str>, i64) = (
            vec![
                ...
                "addx -19", "addx 1", "addx 16", "addx -11", "noop", "noop", "addx 21", "addx -15",
                "noop", "noop", "addx -3", "addx 9", "addx 1", "addx -3", "addx 8", "addx 1",
                "addx 5", "noop", "noop", "noop", "noop", "noop", "addx -36", "noop", "addx 1",
                "addx 7", "noop", "noop", "noop", "addx 2", "addx 6", "noop", "noop", "noop",
                "noop", "noop", "addx 1", "noop", "noop", "addx 7", "addx 1", "noop", "addx -13",
                "addx 13", "addx 7", "noop", "addx 1", "addx -33", "noop", "noop", "noop",
                "addx 2", "noop", "noop", "noop", "addx 8", "noop", "addx -1", "addx 2", "addx 1",
                "noop", "addx 17", "addx -9", "addx 1", "addx 1", "addx -3", "addx 11", "noop",
                ...
                "noop", "addx 9", "noop", "noop", "noop", "addx -1", "addx 2", "addx -37",
                "addx 1", "addx 3", "noop", "addx 15", "addx -21", "addx 22", "addx -6", "addx 1",
                "noop", "addx 2", "addx 1", "noop", "addx -10", "noop", "noop", "addx 20",
